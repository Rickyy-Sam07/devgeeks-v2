  ...
  const LogoCard = ({ logo, rowId }: { logo: any; rowId: string }) => (
    ...
    >
      <div className="w-20 h-20 sm:w-24 sm:h-24 lg:w-28 lg:h-28 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl flex items-center justify-center">
        {logo.image ? (
          <div className="relative w-16 h-8 sm:w-20 sm:h-10 lg:w-24 lg:h-12 bg-gradient-to-r from-purple-500/20 to-blue-500/20 rounded flex items-center justify-center">
            <span className="text-white/60 text-xs">Logo: {logo.name}</span>
          </div>
        ) : logo.bg ? (
